...
export const AiBriefPage: React.FC<AiBriefPageProps> = ({ districts, timelineDays }) => {
  ...
  useEffect(() => {
    ...
    const fetchBrief = async () => {
      ...
      try {
        const res = await axios.get('/api/analysis', {
          params: { district: selectedDistrictId, timeline: timelineDays },
        });
        setBrief(res.data);
      } catch (err) {
    ...
    fetchBrief();
  }, [selectedDistrictId, timelineDays]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="gf-page-header">
        <div>
          <span className="gf-kicker">AI Decision Support</span>
          <h2 className="gf-title mt-2">Executive Brief</h2>
      ...
      {isLoading && (
        <div className="gf-panel flex flex-col items-center justify-center py-20 rounded-2xl">
          ...
          <p className="text-sm font-semibold text-ink">Generating executive brief…</p>
          <p className="text-xs text-muted mt-1">Analyzing satellite data & weather patterns</p>
        </div>
      )}
      ...
      {!isLoading && brief && (
        <div id="executive-brief" className="max-w-4xl mx-auto space-y-6 animate-fade-up">
            ...
            <div className="p-6 sm:p-8">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap items-center gap-3 text-sm text-muted">
                    <span className="flex items-center gap-1.5">
                      ...
                      Horizon <strong className="text-field tabular-nums">+{brief.timelineDays} days</strong>
                    </span>
                    <span className="w-1 h-1 rounded-full bg-line" />
                    <span>{new Date(brief.generatedAt).toLocaleDateString('en-US', { 
                      weekday: 'long', 
                      year: 'numeric', 
                      month: 'long', 
                      day: 'numeric' 
                    ...
                    <span className={`inline-flex items-center gap-1.5 font-semibold ${
                      ...
                      {brief.generatedBy === 'gemini' ? (
                        <>
                          <Brain className="h-4 w-4" /> Gemini AI
                        </>
                      ) : (
                        <>
                ...
                <div className={`shrink-0 rounded-2xl p-4 text-center min-w-[120px] ${
                  ...
                  <p className="text-[11px] uppercase tracking-wider opacity-80 font-semibold">Priority</p>
                  <p className="font-display text-xl font-bold">{brief.priority}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <section className="gf-panel overflow-hidden relative group hover:-translate-y-0.5 transition-all duration-300">
              ...
              <div className="p-6 relative">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="gf-label !text-base !tracking-normal text-field">Recommended Action</h3>
                </div>
            ...
            <section className="gf-panel overflow-hidden relative group hover:-translate-y-0.5 transition-all duration-300">
              ...
              <div className="p-6 relative">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="gf-label !text-base !tracking-normal text-sky">Feed Distribution</h3>
                </div>
          ...
          <div className="text-center py-4 text-xs text-soft space-x-4">
            <span>Generated by GeoForage AI</span>
            <span>·</span>
            <span>{new Date(brief.generatedAt).toISOString()}</span>
            <span>·</span>
            <span>ID: {selectedDistrictId}-{timelineDays}d</span>
          </div>
        </div>
      )}
    </div>
